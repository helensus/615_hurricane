...
#Group2: break the rainfall into 9 parts as required
Floyd_map$cut <- cut(Floyd_map$rain,
                      breaks=c(0,25,50,75,100,125,150,175,200,225),
                      include.lowest = T)

ggplot()+
  #Group2: use fill=cut to show the break
  geom_polygon(data=Floyd_map,mapping=aes(x=long,y=lat,group=group,fill=cut))+
  ...
  #Group2: limitting the range of x and y can make your map in the center
  xlim(min(Floyd_map$long),max(Floyd_map$long))+
  ylim(min(Floyd_map$lat),max(Floyd_map$lat))+
